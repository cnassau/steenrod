...
int piiBasic(primeInfo *pi) {
    ...
    if (NULL==(pi->primpows = (unsigned int*) mallox(sizeof(int) * NALGEXT))) return PI_NOMEM;
    if (NULL==(pi->extdegs  = (unsigned int*) mallox(sizeof(int) * NALGEXT))) return PI_NOMEM;
    if (NULL==(pi->reddegs  = (unsigned int*) mallox(sizeof(int) * NALGEXT))) return PI_NOMEM;
    pi->primpows[0] = 1;
    pi->extdegs[0] = 1;
    pi->reddegs[0] = 1;
    for (i=1; i<NALGEXT; i++) {
...
int piiInv(primeInfo *pi) {
    int i;
    if (NULL == (pi->inverse = (cint*) mallox(sizeof(cint) * pi->prime)))
        return PI_NOMEM;
...
int piiBinom(primeInfo *pi) {
    ...
    cint *dat;
    if (NULL==(dat=pi->binom=(cint*)mallox(sizeof(cint) * pi->prime * pi->prime)))
        return PI_NOMEM;
...
void piiMakeBinom2(primeInfo *pi, int l) {
    ...
    if (NULL == olddat) {
	newdat = (cint*) mallox(sizeof(cint)*2*reqsz);
	*newdat = -1; *(newdat+1) = 1; /* "0 over 0" */
	pi->binom2max = 0;
    } else {
	newdat = (cint*) reallox(olddat,sizeof(cint)*2*reqsz);
    }
...
int SIGNFUNC(unsigned int a, unsigned int b) {
    ...
    while (a) {
        unsigned int z = a ^ (a & (a-1)); /* lowest bit in a */
