...
void ScrobjUpdateproc (Tcl_Obj *objPtr) {
    ...
    Tcl_IncrRefCount(aux[1]);
    Tcl_IncrRefCount(aux[2]);
    Tcl_EvalObjv(tp->ip,3,aux,TCL_EVAL_GLOBAL);
    Tcl_DecrRefCount(aux[0]);
    Tcl_DecrRefCount(aux[1]);
    Tcl_DecrRefCount(aux[2]);
    res = Tcl_GetObjResult(tp->ip);
...
ScrObjType *NewScrobjType(Tcl_Interp *ip, Tcl_Obj *nameobj, Tcl_Obj *update, Tcl_Obj *setany) {
    ...
    res->Update[0] = Tcl_NewStringObj("::apply",-1);
    res->Update[1] = update;
    res->SetAny[0] = Tcl_NewStringObj("::apply",-1);
    res->SetAny[1] = setany;
    res->ip = ip;
    Tcl_IncrRefCount(res->Update[0]);
    Tcl_IncrRefCount(res->Update[1]);
    Tcl_IncrRefCount(res->SetAny[0]);
    Tcl_IncrRefCount(res->SetAny[1]);
    return res;
}
